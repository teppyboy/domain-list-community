...
function processHosts(content, source, sha256) {
...
function matchHash(file, hash, type) {
    let match = false;
    if (type == null) {
        type = "file";
    }
    function getContent() {
        switch (type) {
            ...
            case "string":
                return file;
            default:
                throw new Error("Unknown file type, supported types are 'file' and 'string'");
        }
    }
    try {
        const content = getContent();
        const contentHash = content.split("\n")[2].split(" ")[2];
        if (contentHash === hash) {
            match = true;
        }
    }
    catch (e) {
        console.error("Exception caught during matchHash\n", e);
    }
    finally {
        return match;
    }
...
function convertDir(dir, outDir) {
    ...
    for (let [_, file] of files.entries()) {
        ...
        fs.readFile(fullPath, (_, data) => {
            ...
            function convert() {
                let converted = processHosts(content, fullPath, contentHash);
                fs.writeFileSync(outPath, converted);
                console.log(`Converted ${fullPath} to ${outName}`);
            }
            const updated = matchHash(outPath, contentHash);
            if (updated) {
                console.log(`${fullPath} is up to date.`);
                return;
            }
            convert();
        })
...
function convertUrl(name, url, outDir) {
    ...
    https.request(options, res => {
        ...
        res.on('end', () => {
            ...
            const outPath = path.join(outDir, outName);
            function convert() {
                let converted = processHosts(content, url, contentHash);
                fs.writeFileSync(outPath, converted);
                console.log(`Converted ${name} (${url}) to ${outName}`);
            }
            const updated = matchHash(outPath, contentHash);
            if (updated) {
                console.log(`${name} (${url}) is up to date.`);
                return;
            }
            convert();
        });
